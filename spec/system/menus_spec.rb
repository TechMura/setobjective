  ...
  context "メニューの編集ができるとき" do
    it "ログインしているユーザーは自分のメニューを追加できる" do 
      ...
      options.each_with_index do |option, i|
        if option.value =~ /^[0-6]+$/
          expect{
            ...
            fill_in 'メニュー', with: "#{menu_todo}#{i}"
            find('input[id="add-btn"]').click
            tds = page.all('td')
            expect(tds[i-1]).to have_content("#{menu_todo}#{i}")
      ...
      tds.each.with_index(1)do |td, i|
        expect(td).to have_content("#{menu_todo}#{i}")
      end
    end
  end
...
RSpec.describe "メニューの削除", type: :system do
  ...
  context "メニューの削除ができるとき" do
    it "ログインしているユーザーは自分のメニューを削除できる" do 
      ...
      # 登録されているメニューをすべて削除する
      deleteBtns = page.all('input[value="削除"]')
      deleteBtns.each do |deleteBtn|
        expect{
          deleteBtnValue = deleteBtn.value
          deleteBtn.click
          expect(page).to have_no_content(deleteBtnValue)
        }.to change{ Menu.count }.by(-1)
      end
      click_link '終了'
